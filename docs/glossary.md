...
short on purpose; follow the chapter or [References](references.md) for depth. Terms
...
**Antenna temperature ($T_A$)**{#antenna-temperature}
...
**Aperture efficiency ($\eta_A$)**{#aperture-efficiency}
...
: The width in frequency over which a receiver collects power. Wider bandwidth means more
  collected power and lower noise (see the [radiometer equation](#radiometer-equation)).
...
: The angular response pattern of an antenna or array — how sensitive it is as a function
  of direction. In interferometry you meet several: the **[primary beam](#primary-beam)** (the field of view
  of one dish), the **[synthesised beam](#synthesised-beam)** (the effective resolution of the array, i.e. the
  point-spread function), the **[dirty beam](#dirty-beam-dirty-image)** (that PSF including sidelobes from incomplete
...
: The era ($z \sim 15$–30, observed at ~50–90 MHz) when the first stars switched on and their
  Lyman-α photons coupled the 21 cm [spin temperature](#spin-temperature) to the cold gas, producing the deep
...
: Högbom's iterative deconvolution algorithm: model the sky as a sum of point sources,
  repeatedly subtracting a scaled [dirty beam](#dirty-beam-dirty-image) at the brightest pixel, then restore with a
  clean beam. The workhorse of radio imaging (Chapter 9).

**Closure amplitude**{#closure-amplitude}
: A ratio of four visibility amplitudes ($|V_{ij}||V_{kl}| / |V_{ik}||V_{jl}|$) that cancels
  antenna-based *gain* errors. With [closure phase](#closure-phase), the robust observable that lets sparse VLBI
...
: The relic radiation of the hot early universe — a near-perfect 2.725 K blackbody peaking in
  the microwave. Discovered as excess [antenna temperature](#antenna-temperature) by Penzias & Wilson (1965); see
  Chapter 22.
...
: Recovering the true sky from a dirty image by removing the effect of the [dirty beam](#dirty-beam-dirty-image).
...
**Dirty beam / dirty image**{#dirty-beam-dirty-image}
...
**Effective area ($A_e$)**{#effective-area}
...
**Epoch of Reionization (EoR)**{#epoch-of-reionization}
...
**Fanaroff–Riley class (FR I / FR II)**{#fanaroff-riley-class}
...
: The rotation of a linearly polarised wave's plane of polarisation as it passes through a
  ...
  dependence is the signature that lets the **[rotation measure](#rotation-measure)** be recovered (Chapter 37).

**Fast radio burst (FRB)**{#fast-radio-burst}
...
**Free-free (thermal bremsstrahlung)**{#free-free}
: The thermal continuum radiated by free electrons scattering off ions in ionized gas (HII
  ...
  **flat**: optically thick at low frequency ($T_B \to T_e$, $S_\nu \propto \nu^2$) and
  optically thin above the turnover, where the [spectral index](#spectral-index) flattens to $\alpha \approx -0.1$.
...
: The width of a peak (a beam, a [spectral line](#spectral-line), a pulse) measured between the points where
...
**Hanbury Brown–Twiss (HBT) / intensity interferometry**{#hanbury-brown-twiss}
...
**Hellings–Downs curve**{#hellings-downs-curve}
: The characteristic quadrupolar correlation between pulsar pairs as a function of their angular
  separation — the fingerprint of a gravitational-wave background in a [pulsar timing array](#pulsar-timing-array)
  (Chapter 20).

**HI / 21 cm line**{#hi-21-cm-line}
: The [spectral line](#spectral-line) of neutral atomic hydrogen at 1420.4 MHz (wavelength 21 cm), from the
...
**Inverse-Compton catastrophe**{#inverse-compton-catastrophe}
: The runaway cooling that caps an incoherent synchrotron source's [brightness temperature](#brightness-temperature) at
...
**Jansky (Jy)**{#jansky}
: The unit of [flux density](#flux-density), $1\,\mathrm{Jy} = 10^{-26}\,\mathrm{W\,m^{-2}\,Hz^{-1}}$. Named
...
**Long-period radio transient (LPT)**{#long-period-radio-transient}
...
**log N–log S (source counts)**{#log-n-log-s}
: The number of radio sources brighter than [flux density](#flux-density) $S$ as a function of $S$. In a static,
...
**Macquart relation**{#macquart-relation}
: The roughly linear relation between a [fast radio burst](#fast-radio-burst)'s [dispersion measure](#dispersion-measure) and its redshift,
...
**Measurement Set (MS)**{#measurement-set}
...
**Noise temperature ($T_N$)**{#noise-temperature}
: The temperature of a fictitious resistor that would generate the same noise power as a
  ...
  the [system temperature](#system-temperature).
...
**Perceptually-uniform colormap**{#perceptually-uniform-colormap}
...
**Plasma frequency ($f_p$)**{#plasma-frequency}
...
**Polarisation / Stokes parameters**{#polarisation-stokes-parameters}
...
**Polarisation fraction & angle**{#polarisation-fraction-angle}
...
**Radio luminosity function**{#radio-luminosity-function}
: The space density of radio sources per unit (logarithmic) luminosity. It is steeply
  ...
  rare, powerful [radio-loud AGN](#radio-loud-agn). Its convolution with cosmic evolution shapes the observed
...
**Radio-quiet zone**{#radio-quiet-zone}
...
**Radio recombination line (RRL)**{#radio-recombination-line}
: A [spectral line](#spectral-line) emitted when an electron recombines into a high-$n$ level of an ion and
...
: The Receiver Operating Characteristic plots a classifier's true-positive rate against its
  ...
  e.g. a learned classifier vs a [matched filter](#matched-filter) (Chapter 38).

**RASDR (Radio Astronomy SDR)**{#rasdr}
...
**RM synthesis (Faraday tomography)**{#rm-synthesis}
...
**Rotation measure (RM)**{#rotation-measure}
: The constant of proportionality in **[Faraday rotation](#faraday-rotation)**, $\Delta\chi = \mathrm{RM}\,\lambda^2$,
...
**SED (Saturn Electrostatic Discharge)**{#sed}
...
: The [flux density](#flux-density) of a source that would double the system noise, $\mathrm{SEFD} =
...
: Using a strong source in the field itself to solve for time-varying antenna gains and
  ...
  improves [dynamic range](#dynamic-range) when closure relations constrain the solution.
...
**Spectral index ($\alpha$)**{#spectral-index}
...
**Spin temperature ($T_S$)**{#spin-temperature}
: The excitation temperature of the 21 cm hyperfine levels, set by a weighted mean of the CMB,
  ...
  [Wouthuysen–Field effect](#wouthuysen-field-effect)). Whether the cosmological [21 cm line](#hi-21-cm-line) is seen in absorption or emission
...
**Stokes parameters ($I, Q, U, V$)**{#stokes-parameters}
: See **[Polarisation / Stokes parameters](#polarisation-stokes-parameters)** (under P) — the four numbers that fully describe a
...
**Synchrotron self-absorption (SSA)**{#synchrotron-self-absorption}
...
**Synthesised beam**{#synthesised-beam}
: The effective point-spread function of an interferometer, set by its *uv* coverage — the
  array's "resolution element". See **[Beam](#beam)** and Chapter 8.

**System temperature ($T_{\text{sys}}$)**{#system-temperature}
...
**Tapering / weighting**{#tapering-weighting}
...
**TOGA (Time of Group Arrival)**{#toga}
...
**Wouthuysen–Field effect**{#wouthuysen-field-effect}
